"""
AI Daily Briefing Generator — Claude API
==========================================
사용법:
...
# Claude API 전용: 뉴스 분석
# ============================================================
def collect_and_analyze_news(client: anthropic.Anthropic) -> tuple[str, list]:
    today = get_today_str()
    ...
    return "".join(b.text for b in response.content if hasattr(b, "text")), articles


def analyze_it_company_news(client: anthropic.Anthropic, articles: list) -> tuple[str, list]:
    today = get_today_str()
    news_window = get_news_window_str()
...
def fix_json_with_claude(raw_text: str) -> dict:
    ...
    fixed_text = "".join(b.text for b in response.content if hasattr(b, "text"))
...
def main():
    ...
    print("📋 뉴스 데이터 파싱 중...")
    try:
        news_data = parse_news_json(raw_response, fix_fn=fix_json_with_claude)
    ...
    try:
        ...
        if it_articles:
            ...
            it_data = parse_news_json(it_raw, fix_fn=fix_json_with_claude)
