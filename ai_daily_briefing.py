...
============================
Claude API의 web_search 도구를 활용해 AI 뉴스를 자동 수집하고
HTML 뉴스레터를 생성하는 반자동화 스크립트

사용법:
  1. pip install anthropic
  2. export ANTHROPIC_API_KEY="your-api-key"
  3. python ai_daily_briefing.py
  4. output/ 디렉토리에 HTML 파일 생성됨

비용 추정 (1회 실행):
  - Web Search: ~5-10회 검색 × $0.01 = $0.05~$0.10
  - Claude Sonnet 4.6 토큰: 입력 ~5K + 출력 ~4K ≈ $0.03~$0.05
  - 총 1회 실행 비용: 약 $0.08~$0.15 (≈ 100~200원)
"""
...
def parse_news_json(raw_text: str) -> dict:
    ...
    if json_match:
        ...
        for i, ch in enumerate(raw_text):
            ...
            raise ValueError("JSON을 찾을 수 없습니다. Claude 응답을 확인하세요.")
    ...
    try:
        ...
        print("  🔧 Claude에게 JSON 수정 요청 중...")
        return fix_json_with_claude(raw_text)


def fix_json_string(json_str: str) -> str:
    """흔한 JSON 깨짐 패턴 자동 수정"""
    # 1) 문자열 값 안의 이스케이프 안 된 줄바꿈 제거
    # JSON 문자열 내부의 실제 개행을 \n으로 교체
    lines = json_str.split('\n')
    json_str = '\n'.join(lines)

    # 2) HTML 태그 안의 따옴표가 JSON을 깨뜨리는 경우 처리
    # <strong class="..."> 같은 패턴의 따옴표를 이스케이프
    json_str = re.sub(
        r'<(\w+)\s+(\w+)="([^"]*)"',
        r'<\1 \2=\'\3\'',
        json_str
    ...
    # 4) 후행 쉼표 제거 (JSON에서 허용 안 됨)
    json_str = re.sub(r',\s*([\]}])', r'\1', json_str)

    return json_str


def fix_json_with_claude(raw_text: str) -> dict:
    """파싱 실패 시 Claude에게 JSON 수정 요청 (fallback)"""
    client = anthropic.Anthropic()

    # 원본 텍스트에서 JSON 부분만 추출 (앞뒤 5000자)
    ...
    response = client.messages.create(
        ...
        messages=[
            {
                "role": "user",
                "content": f"""아래 텍스트에서 JSON 부분을 추출하고, 문법 오류를 수정해서 
...
def main():
    print("=" * 60)
    print(f"  AI Daily Briefing Generator")
    ...
    try:
        ...
        print("   이 파일을 Claude에 보내주시면 디버깅 도와드릴게요.")
        return
